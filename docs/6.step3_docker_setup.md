...
## 🐳 Background: Understanding Docker

### Without Docker

```
Development → pip install → venv setup → Works on my machine
             → Deployment → "It works on my machine!" problem
```

### With Docker

```
Code + Dockerfile → Build Image → Run Container → Same everywhere
```
...
docker run -e DEBUG=true -p 8000:8000 myimage

# Run with volume
docker run -v $(pwd):/app -p 8000:8000 myimage
```
...
# Use different port
docker run -p 8001:8000 searchflow:latest
```
...
docker run --network searchflow-net -p 8000:8000 searchflow:latest
```
